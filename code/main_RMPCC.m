...
for k = [1:50] 
    ...
    if exit2~=1
       c0(n+1:n+3*m+p)=zeros(3*m+p,1);  
    else
       c0(n+1:n+m)=y0;
       c0(n+m+1:n+m+p)= lambda2.ineqlin;
       c0(n+m+p+1:n+2*m+p)= lambda2.upper;
       c0(n+2*m+p+1:n+3*m+p)= lambda2.lower;
    end

    for j=0:Max_iter
        options = optimoptions('fmincon','Algorithm','sqp','TolFun',epsilon_SQP);
        [x,ObjVal]=fmincon(@(x)fun(x),c0,[],[],[],[],[],[],@(x)constraints_RMPCC(x,t0),options);    
        b=norm( x(n+m+1:n+m+p)'*(A_2*x(1:n)+B_2*x(n+1:n+m)-b_2)+x(n+m+p+1:n+2*m+p)'*(x(n+1:n+m)-ub_y)+x(n+2*m+p+1:n+3*m+p)'*(lb_y-x(n+1:n+m)));
        if b<=epsilon_r
        break;
        else
           c1(1:n)=x(1:n);
           ...
           if exit3~=1
              ...
              c1(n+1:n+m)=z1;
              c1(n+m+1:n+m+p)= lambda3.ineqlin;
              c1(n+m+p+1:n+2*m+p)= lambda3.upper;
              c1(n+2*m+p+1:n+3*m+p)= lambda3.lower;
           end
        c0=c1;
