...
for k = [1:50] 
    ...
    [x,ObjVal]=fmincon(@(x)fun(x),c0,[],[],[],[],[],[],@(x)constraints_RWDP(x,t0),options);
    b=norm( d_2'*x(n+1:n+m)-d_2'*x(n+m+1:n+2*m)-x(n+2*m+1:n+2*m+p)'*(A_2*x(1:n)+B_2*x(n+m+1:n+2*m)-b_2)-...
            x(n+2*m+p+1:n+3*m+p)'*(x(n+m+1:n+2*m)-ub_y)-x(n+3*m+p+1:n+4*m+p)'*(lb_y-x(n+m+1:n+2*m)));
    if b<=epsilon_r
    break;
    else
       c1(1:n)=x(1:n);
