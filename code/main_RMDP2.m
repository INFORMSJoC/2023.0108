...
for k = [1:50] 
    ...
    for j=0:Max_iter  
        ...
        [x,ObjVal]=fmincon(@(x)fun(x),c0,[],[],[],[],[],[],@(x)constraints_RMDP2(x,t0),options);   
        b=norm(-x(n+2*m+1:n+2*m+p)'*(A_2*x(1:n)+B_2*x(n+m+1:n+2*m)-b_2)-x(n+2*m+p+1:n+3*m+p)'*(x(n+m+1:n+2*m)-ub_y)-x(n+3*m+p+1:n+4*m+p)'*(lb_y-x(n+m+1:n+2*m)));
        if b<=epsilon_r
        break;
        else
          c1(1:n)=x(1:n);
